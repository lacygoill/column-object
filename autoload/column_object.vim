...
fu s:find_boundary_lines(lnum, indent, col, vcol, dir) abort "{{{1
    ...
    let is_code = synID(cur_lnum, a:col, 1)
        \ ->synIDtrans()
        \ ->synIDattr('name') isnot# 'Comment'
    while cur_lnum != limit
        ...
        let is_relevant = is_code && synID(next_lnum, a:col, 1)
            \ ->synIDtrans()
            \ ->synIDattr('name') isnot# 'Comment'
            \ || !is_code && synID(next_lnum, a:col, 1)
            \ ->synIDtrans()
            \ ->synIDattr('name') is# 'Comment'
...
fu s:find_boundary_columns(top_line, bottom_line, vcol, iw_aw, on_space) abort "{{{1
    ...
    while lnum <= a:bottom_line
        ...
        if [vcol1, vcol2] ==# [-1, -1]
            ...
            let word_selected_is_not_empty = getline('.')
